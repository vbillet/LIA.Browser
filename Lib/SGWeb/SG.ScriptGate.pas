...
begin
  ...
  begin
    ...
    Factors := iURL.Split( [PARAM_SEP, METHOD_END_SEP], TStringSplitOptions.ExcludeEmpty);

    RttiType := SharedContext.GetType(FReceiver.ClassType);
    if (RttiType = nil) then
    begin
      CallErrorEvent(TScriptGateTargetTypeNotFound.Create('Target type not found:' + FReceiver.ClassName));
      Exit;
    end;
    ...
    begin
      CallErrorEvent(TScriptGateMethodNotFound.Create('Method not found: ' + Method));
      Exit;
    end;
    ...
    begin
      CallErrorEvent(TScriptGateParameterCountMismatch.Create('Parameter count mismatch.'));
      Exit;
    end;
...
begin
  ...
  LogD('=== TScriptGate.CheckScheme.URL = ' + iURL + ' ' + FScheme + SCHEME_SEP);

  Result := iURL.StartsWith(FScheme + SCHEME_SEP) or Done;
  if Result then LogD('OK') else LogD('NOK');
  Result := true;
end;
